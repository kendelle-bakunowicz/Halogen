...
bool DrawnPieceCombination(const Position& position);
...
int EvaluatePositionNet(Position& position, EvalCacheTable& evalTable)
{
    ...
    if (!evalTable.GetEntry(position.GetZobristKey(), eval))
    {
        ...
        if (DrawnPieceCombination(position))
        {
            //If the winning side has the wrong piece combination and no pawns then divide the score by a factor of 16.
            if (eval > 0 && position.GetPieceBB(WHITE_PAWN) == 0)
                eval /= 16;
            if (eval < 0 && position.GetPieceBB(BLACK_PAWN) == 0)
                eval /= 16;
        }
