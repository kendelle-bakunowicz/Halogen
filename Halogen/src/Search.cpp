...
uint64_t BenchSearch(const Position& position, int maxSearchDepth)
{
	...
	SearchPosition(positionCopy, sharedData, 0, 2147483647, 2147483647, maxSearchDepth);
...
void MateSearch(const Position& position, int searchTime, int mate)
{
	...
	Position positionCopy(position);
	SearchPosition(positionCopy, sharedData, 0, searchTime, searchTime, MAX_DEPTH, mate);
	PrintBestMove(sharedData.GetBestMove());
}
