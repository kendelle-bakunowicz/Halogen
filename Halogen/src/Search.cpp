		...
		if (moves[i].IsPromotion())
		{
			if (!IsSquareThreatened(position, moves[i].GetTo(), position.GetTurn()))
			{
				if (moves[i].GetFlag() == KNIGHT_PROMOTION || moves[i].GetFlag() == KNIGHT_PROMOTION_CAPTURE)
					moves[i].SEE += RelativePieceValues[WHITE_KNIGHT] - RelativePieceValues[WHITE_PAWN];
				if (moves[i].GetFlag() == ROOK_PROMOTION || moves[i].GetFlag() == ROOK_PROMOTION_CAPTURE)
					moves[i].SEE += RelativePieceValues[WHITE_ROOK] - RelativePieceValues[WHITE_PAWN];
				if (moves[i].GetFlag() == BISHOP_PROMOTION || moves[i].GetFlag() == BISHOP_PROMOTION_CAPTURE)
					moves[i].SEE += RelativePieceValues[WHITE_BISHOP] - RelativePieceValues[WHITE_PAWN];
				if (moves[i].GetFlag() == QUEEN_PROMOTION || moves[i].GetFlag() == QUEEN_PROMOTION_CAPTURE)
					moves[i].SEE += RelativePieceValues[WHITE_QUEEN] - RelativePieceValues[WHITE_PAWN];
			}
			else
			{
				moves[i].SEE -= RelativePieceValues[WHITE_PAWN];	//if its defended then end result is we are just going to lose my pawn
			}
		}
	}
