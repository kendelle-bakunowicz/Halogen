...
SearchResult NegaScout(Position& position, unsigned int initialDepth, int depthRemaining, int alpha, int beta, int colour, unsigned int distanceFromRoot, bool singleNull, bool doubleNull, SearchData& locals, ThreadSharedData& sharedData);
...
SearchResult AspirationWindowSearch(Position& position, int depth, int prevScore, SearchData& locals, ThreadSharedData& sharedData, unsigned int threadID, Timer& searchTime)
{
	...
	while (!locals.AbortSearch(0) || depth == 1)
	{
		...
		search = NegaScout(position, depth, depth, alpha, beta, position.GetTurn() ? 1 : -1, 0, false, false, locals, sharedData);
...
SearchResult NegaScout(Position& position, unsigned int initialDepth, int depthRemaining, int alpha, int beta, int colour, unsigned int distanceFromRoot, bool singleNull, bool doubleNull, SearchData& locals, ThreadSharedData& sharedData)
{
	...
	/*Null move pruning*/
	if (AllowedNull(!doubleNull, position, beta, alpha, InCheck) && (singleNull || staticScore > beta))	//allow double null to bypass the evaluation requirement
	{
		unsigned int reduction = R + (depthRemaining >= static_cast<int>(VariableNullDepth));

		position.ApplyNullMove();
		int score = -NegaScout(position, initialDepth, depthRemaining - reduction - 1, -beta, -beta + 1, -colour, distanceFromRoot + 1, !singleNull, singleNull, locals, sharedData).GetScore();
	...
	if (!hashMove.IsUninitialized() && position.GetFiftyMoveCount() < 100 && MoveIsLegal(position, hashMove))	//if its 50 move rule we need to skip this and figure out if its checkmate or draw below
	{
		...
		int newScore = -NegaScout(position, initialDepth, extendedDepth - 1, -b, -a, -colour, distanceFromRoot + 1, false, false, locals, sharedData).GetScore();
		position.RevertMove();
	...
	for (size_t i = 0; i < moves.size(); i++)	
	{
		...
		if (LMR(InCheck, position) && i > 3)
		{
			...
			int score = -NegaScout(position, initialDepth, extendedDepth - 1 - reduction, -a - 1, -a, -colour, distanceFromRoot + 1, false, false, locals, sharedData).GetScore();
		...
		{	
			newScore = -NegaScout(position, initialDepth, extendedDepth - 1, -beta, -a, -colour, distanceFromRoot + 1, true, false, locals, sharedData).GetScore();
		}
